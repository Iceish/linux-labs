...
if [[ "$(cat right-reactor/engine.conf 2>/dev/null  | tr -d '\n')" == "fuel-type=tritiumquantity=39KBengine-with-starter=1" || "$(cat right-reactor/engine.conf 2>/dev/null  | tr -d '\n')" == "fuel-type=tritiumquantity=40KBengine-with-starter=1" ]]; then
